---
name: knowledge
description: Extracts knowledge from Amp threads and updates project docs. Use when asked to document recent work, sync docs after an epic, summarize what changed, or update AGENTS.md from thread history. ALWAYS generate content in English.
---
...
| "Changed pattern" | `finder` or `Grep` with regex               |
...
| Semantic search     | `finder` or `Grep` with regex                          |
...
```
- [ ] Output is in English (regardless of prompt language)
